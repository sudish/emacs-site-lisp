...
(defvar rails-directories
  (list "app" "spec" "test" "db" "config" "lib")
...
(defun rails-make-displayable-name (path)
  ...
  (let* ((dir (file-name-directory path))
	 ...
	 (type (cond
		...
		((string-match "/app/controllers/\\([a-zA-Z0-9_/]+\\)/" dir)
		 (concat "controller(" (match-string 1 dir) ")"))
		((string-match "/app/models/" dir) "model")
		((string-match "/lib/\\([a-zA-Z0-9_/]+\\)/" dir)
		 (concat "lib(" (match-string 1 dir) ")"))
		((string-match "/lib/" dir) "lib")
		((string-match "/db/\\([a-zA-Z0-9_/]+\\)/" dir)
		 (concat "db(" (match-string 1 dir) ")"))
		((string-match "/db/" dir) "db")
		((string-match "/config/\\([a-zA-Z0-9_]+\\)/" dir)
		 (concat "config(" (match-string 1 dir) ")"))
		((string-match "/config/" dir) "config")
...
(defun rails-list-project-files (&optional directory)
  ...
  (let ((rails-project-root (locate-dominating-file (or directory default-directory) "Gemfile")))
