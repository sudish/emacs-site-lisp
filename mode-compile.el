...
;;
;;   Author: Heddy Boubaker <boubaker@cena.dgac.fr>
;;   Maintainer: Heddy Boubaker <boubaker@cena.dgac.fr>
;;   Created: June 1994
;;   Last modified: 1997/04/08 15:00:32
;;   Version: 2.20
;;   Keywords: compile, modes
;;   Tested for:
;;     XEmacs (Lucid GNU Emacs) >= 19.10 
;;     Must work with FSF GNU Emacs > 19.31 ;-)
;;     Seems to not work anymore for Emacses <= 18 ???
;;   Ftp access: 
;;    ftp.cenatls.cena.dgac.fr:pub/Emacs/mode-compile-<v number>.el.gz
...
;; 1997/04/08 15:00:32|2.20|~/misc/mode-compile.el.Z|
;;
...
;;  On X-Windows systems setting the variable `mode-compile-other-frame-p'
...
;;          mc--frame-live-p, mc--select-frame, mc--make-frame,
;;          mc--make-frame-visible, mc--raise-frame, mc--member,
;;          mc--run-hooks, mc--compile-sav
...
;; ===========
;;
;;   Bug when compile-command is local-variable corrected
;;   Tcl/Tk support added - JWH
;;   Misc portability improvements
;;   Bug correction: void mode-compile-object-file-ext
;;   Bug correction when running under a MSDOSish file system
;;   Adding minimal support for running commands remotely C-uC-cc
;; unlike rcompile there is no support for ange-ftp.
;;
...
A new Emacs FRAME is created and the compilation command is executed in this
other frame.
To specify the frame parameters see also variable
`mode-compile-frame-parameters-alist'.")
(defalias 'mode-compile-other-screen-p 'mode-compile-other-frame-p)

(defvar   mode-compile-other-frame-name
  "COMPILATION"
...
(defalias 'mode-compile-other-screen-name 'mode-compile-other-frame-name)
...
(defalias 'mode-compile-default-screen-parameters 'mode-compile-default-frame-parameters)
...
See also variable `mode-compile-default-frame-parameters'
and `mode-compile-other-frame-name'.
...
")
(defalias 'mode-compile-screen-parameters-alist 'mode-compile-frame-parameters-alist)
...
(defvar mode-compile-exe-file-ext
  (cond
   ((memq system-type '(ms-dos emx windows-nt)) ".exe")
   (t ""))
...
(defvar mode-compile-remote-hosts-alist '()
  "Alist of favourites hosts names and the username
to use to log on (HOSTNAME . USERNAME).
 
If USERNAME is nil the function user-login-name will be used.
")

(defvar mode-compile-remote-execute-command "rsh"
  "The shell command used to run a command remotely.
\"rsh\" is the only choice I know but I'm far to know everything...
")

(defvar mode-compile-remote-execute-set-host-arg nil
...
(defvar mode-compile-remote-execute-set-command-arg nil
...
(defvar cc-build-output-args t
  "Build output-args for c-mode.
")

(defvar cc-object-file-ext "o"
...
(defvar   java-compiler-varenv
  "JAVAC"
  "Varenv indicating the C compiler to use.
")

(defvar   java-cflags-varenv
  "JAVAC_FLAGS"
  "Varenv indicating the C compiler flags to use.
")

(defvar   java-source-ext-list
  '( "java" )
  "Extensions for Java compileable source files.
")

(defvar   java-headers-ext-list
  '( "java" )
  "Extensions for Java source files.
")

(defvar java-default-compiler-options
  "-O"
...
(defvar java-source-file-ext-regexp   
  "\\.java" 
  "Regexp to find, from it's name, if a Java file is compileable.
")

(defvar java-build-output-args nil
  "Dont build output-args for Java-mode.
")

(defvar java-object-file-ext "class"
  "Extension of objects file (result of compilation)
in java mode.
") 
...
(defvar c++-build-output-args t
  "Build output-args for c++-mode.
")

(defvar c++-object-file-ext "o"
  "Extension of objects file (result of compilation)
in c++ mode.
") 
...
(defvar ada-build-output-args t
  "Build output-args for ada-mode.
")

(defvar ada-object-file-ext "o"
  "Extension of objects file (result of compilation)
in ada mode.
") 
...
(defvar f77-build-output-args t
  "Build output-args for f77-mode.
")

(defvar f77-object-file-ext "o"
  "Extension of objects file (result of compilation)
in Fortran mode.
") 
...
;;;###autoload
(defconst mode-compile-version "2.20"
  "Current version of mode-compile package.
 
mode-compile.el,v 2.20 1997/04/08 15:00:32 boubaker Exp
Please send bugs-fixes/contributions/comments to boubaker@cena.dgac.fr
")

(defconst mode-compile-help-address "boubaker@cena.dgac.fr"
...
(defun mc--set-remote-cmd (remote-host)
  ...
  (if (stringp remote-host)
      ...
      (let ((host-infos (assoc remote-host
        ...
        (setq mc--remote-username
              (or (cdr host-infos) (user-login-name)))
    ...
    (let ((rhost (completing-read
                  ...
                  ;; Initial contents
                  (or mc--remote-host
                      (caar mode-compile-remote-hosts-alist)))))
      (or (string= rhost "")
...
(defun tcl-compile ()
  ;; JWH
  "Run \"wish\" with `tcl-dbg-flags' on current-buffer (`tcl-mode').
...
See variables compilation-error-regexp-alist or tcl-compilation-error-regexp-alist."
  (mc--shell-compile "wish" tcl-dbg-flags tcl-compilation-error-regexp-alist))
...
(defun mode-compile-submit-bug-report ()
  "*Submit via mail a bug report on mode-compile v2.20."
  (interactive)
  (and
